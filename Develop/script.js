 ...
 // Then I call each iteam in the Arrray with .map and select the index to and added 97 to each value, to match up with the code for the letters in javascript. - shout out Denis.

 var characterCode = Array.from(Array(26)).map((_,i) => i + 97);

  // Here for to illistrate how to write out funtion with out arrow.
  //var characterCodes= Array.from(Array(26)).map(function(_,i){
  //i+97;
  // })
  // console.log("data",characterCode)
 ...
 var lowerCaseLetters = characterCode.map(char => String.fromCharCode(char))
...
function availibleCharacters (character) {
  if(withNumbers){
    return numbers
  }else{
    return []
